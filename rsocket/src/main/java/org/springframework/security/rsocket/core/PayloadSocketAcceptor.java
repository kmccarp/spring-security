...
.parseMimeType(WellKnownMimeType.MESSAGE_RSOCKET_COMPOSITE_METADATA.getString());
	...
	metadataMimeType)
	.flatMap(
(ctx) -> this.delegate.accept(setup, sendingSocket).map((acceptingSocket) -> new PayloadInterceptorRSocket(acceptingSocket,
												this.interceptors, metadataMimeType, dataMimeType, ctx)).contextWrite(ctx));
	}
